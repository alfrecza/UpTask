...
const Registrar = () => {
  ...
  const handleSubmit = async (e) => {
      ...
      if(password.length < 6) {
        setAlerta({
          msg: "El password es muy corto, deben ser como mínimo 6 caracteres",
          error: true
        })
        return
      }
  ...
  return (
    <>
      <h1 className="text-sky-600 font-black text-6xl capitalize">Crea tu cuenta y administra tus <span className="text-gray-700 ">proyectos</span></h1>
      ...
      <form action="" className="my-10 bg-white shadow rounded-lg p-10" onSubmit={handleSubmit}>
        <div className="my-5">
          ...
          <input type="text" id="nombre" placeholder="Tu nombre" className="w-full mt-3 p-3 border rounded-xl bg-gray-50 outline-sky-600" value={nombre} onChange={e => setNombre(e.target.value)}/>
        </div>
        <div className="my-5">
        ...
        <div className="my-5">
          ...
          <input type="password" id="password2" placeholder="Repetir tu password" className="w-full mt-3 p-3 border rounded-xl bg-gray-50 outline-sky-600" value={repetirPassword} onChange={e => setRepetirPassword(e.target.value)}/>
        </div>
